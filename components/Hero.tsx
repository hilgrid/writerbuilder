...
const Hero: React.FC<HeroProps> = ({ onNavigate }) => {
  const handleSteeringWheelClick = () => {
    onNavigate('steering-wheel');
  };
  ...
  return (
    <section className="relative min-h-screen w-full flex flex-col items-center justify-center vintage-bg overflow-hidden p-4">
      ...
      <div className="flex flex-col space-y-2 mt-8 z-20">
        <a href="https://hils.substack.com" target="_blank" rel="noopener noreferrer" className="text-stone-800 text-base hover:text-stone-600 transition-colors">
          Writerbuilder newsletter
        </a>
        <a href="https://maven.com/hilary-gridley/ai-powered-people-management" target="_blank" rel="noopener noreferrer" className="text-stone-800 text-base hover:text-stone-600 transition-colors">
          Supermanagers course
        </a>
        <button onClick={handleSteeringWheelClick} className="text-stone-800 text-base hover:text-stone-600 transition-colors text-left cursor-pointer">
          AI steering wheel
        </button>
      </div>
    </section>
